...
class EdgeClassifier(nn.Module):
    def __init__(self):
        ...
        self.enc1_1 = nn.Conv2d(5, 128, kernel_size=3, stride=2)
        self.enc1_2 = nn.Conv2d(128, 128, kernel_size=3, stride=2)
        self.enc1_3 = nn.Conv2d(128, 128, kernel_size=3, stride=2)
        self.enc1_4 = nn.Conv2d(128, 128, kernel_size=3, stride=2)
        self.enc1_5 = nn.Conv2d(128, 128, kernel_size=3, stride=2)
        ...
        self.enc2_1 = nn.Linear(128, 512, bias=True)
        self.enc2_2 = nn.Linear(512, 512, bias=True)
        self.enc2_3 = nn.Linear(512, 512, bias=True)
        self.enc2_4 = nn.Linear(512, 1, bias=True)
        #self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, x):
        ...
        # corners updates        
        x_s = F.relu(self.enc1_1(x))
        x_s = F.relu(self.enc1_2(x_s))
        x_s = F.relu(self.enc1_3(x_s))
        x_s = F.relu(self.enc1_4(x_s))
        x_s = F.relu(self.enc1_5(x_s))
        x_s = F.relu(self.enc1_6(x_s))

        x_s = x_s.view(-1, 128)
        x_s = F.relu(self.enc2_1(x_s))
        x_s = F.relu(self.enc2_2(x_s))
        x_s = F.relu(self.enc2_3(x_s))
        logits = self.enc2_4(x_s)
