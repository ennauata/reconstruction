...
class GraphData(Dataset):
    ...
    def change_state_check(self, imgs, edges_state, edges_gt, e_xys, k):
        ...
        return sample, label, prob, dist_from_gt
    ...
    def change_state_check(self, imgs, edges_state, edges_gt, e_xys, k):
        ...
        with torch.no_grad():
            ...
            prob = probs[0][1]
            dist_from_gt = abs(label-prob) # try to add more samples based on this diff (hard negative mining)
        return sample, sample_rev, label, label_rev, prob, dist_from_gt

    def generate_input_tuple(self, corners_gt, edges_gt, e_xys, corner_edge, current_edges, imgs, n_max=1024):
        ...
        # get samples flipping each edge
        aux_current_edges = np.zeros_like(current_edges)
        for k in range(aux_current_edges.shape[0]):

            # draw current state
            im_s0 = self.draw_edges(aux_current_edges, e_xys)

            # draw new state
            new_state = np.array(aux_current_edges)
            new_state[k] = abs(1.0-new_state[k])
            im_s1 = self.draw_edges(new_state, e_xys)
            sample = np.concatenate([imgs.transpose(2, 0, 1)/255.0, im_s0[np.newaxis, :, :], im_s1[np.newaxis, :, :]])
            sample_rev = np.concatenate([imgs.transpose(2, 0, 1)/255.0, im_s1[np.newaxis, :, :], im_s0[np.newaxis, :, :]])
            ...
            # update using oracle
            aux_current_edges[k] = edges_gt[k]

        ############################
        ### SAMPLING - Method #2 ###
        ############################

        # get samples using CNN
        predicted_edges = np.zeros_like(current_edges)
        count = 0 

        # # select edges subset
        # pos_inds = np.where(edges_gt == 1)[0]
        # neg_inds = np.where(edges_gt == 0)[0] 
        # neg_inds = np.random.choice(neg_inds, min(pos_inds.shape[0], neg_inds.shape[0]))
        # inds = np.concatenate([pos_inds, neg_inds, np.array([0])]) # add first index (make sure we always pass the complete polygon)
        # np.random.shuffle(inds)
        for _ in range(current_edges.shape[0]):
            keep_track_cnn = (-1, -1, None, None, None, None)
            for k in range(current_edges.shape[0]):

                # check next states
                sample_with_cnn, sample_with_cnn_rev, label_with_cnn, label_with_cnn_rev, prob_with_cnn, dist_with_cnn = self.change_state_check(imgs, predicted_edges, edges_gt, e_xys, k)
                
                # keep track
                if prob_with_cnn > keep_track_cnn[0]:
                     keep_track_cnn = (prob_with_cnn, k, sample_with_cnn, label_with_cnn, sample_with_cnn_rev, label_with_cnn_rev)

                # hard negative mining
                if dist_with_cnn > .8 and count < 2:
                    samples.append(sample_with_cnn)
                    targets.append(label_with_cnn)
                    samples.append(sample_with_cnn_rev)
                    targets.append(label_with_cnn_rev)
                    count += 1

                    # Debug
                    # imgs_deb = (keep_track_cnn[2][:3, :, :].transpose(1, 2, 0) * 255.0).astype('uint8')
                    # im_s0_deb  = keep_track_cnn[2][4, :, :]
                    # im_s1_deb  = keep_track_cnn[2][5, :, :]
                    # imgs_deb = (sample_with_cnn[:3, :, :].transpose(1, 2, 0) * 255.0).astype('uint8')
                    # im_s0_deb  = sample_with_cnn[4, :, :]
                    # im_s1_deb  = sample_with_cnn[5, :, :]

                    # print('A')
                    # rgb = Image.fromarray(imgs_deb)
                    # im0 = Image.fromarray((im_s0_deb>0)*255.0)
                    # im1 = Image.fromarray((im_s1_deb>0)*255.0)
                    # print(dist_with_cnn)
                    # plt.figure()
                    # plt.imshow(rgb)
                    # plt.figure()
            ...
            # update states using the cnn best action
            if keep_track_cnn[0] > .5:
                predicted_edges[keep_track_cnn[1]] = abs(1.0-predicted_edges[keep_track_cnn[1]])      
            
            # add samples
            # samples.append(keep_track_cnn[2])
            # targets.append(keep_track_cnn[3])
        ...
        # # select some samples
        # inds = list(range(samples.shape[0]))
        # np.random.shuffle(inds)
        # inds = inds[:n_max]
