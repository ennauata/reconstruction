...
from model.resnet import ResNetLoop, GraphModelCustom
...
def main(options):
    ...
    if False:
        ...
        edge_confidence = building.edges_gt.astype(np.float32)
        edge_confidence += np.random.random(len(edge_confidence)) * 0
        # edge_state[1] = 0.3
        # edge_state[2] = 0.3
        # edge_state[3] = 0.6
        # edge_state[4] = 0.6
    ...
    if 'graph' in options.suffix:
        ...
        options.suffix = options.suffix[:-4]
    else:
        model = ResNetLoop(options)
        pass
