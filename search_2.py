...
from model.regressor import EdgeClassifier as DistanceRegressor
import time
...
for l, data in enumerate(valid_loader):
    ...
    _id = valid_list[l]
    count = 0
    corners_det, edges_det, corner_edge, e_xys, init_edges = data
    ...
    # init states
    init_current_edges = np.zeros(corner_edge.shape[1])  #np.array(init_edges.view(-1)) #np.zeros(corner_edge.shape[1]) 
    init_track_edges = np.zeros(corner_edge.shape[1])
    ...
    states = [(init_current_edges, init_track_edges)]
    while len(states) > 0:
        ...
        # retrieve next state
        current_edges, track_edges = states.pop()

        # draw current state
        im_s0 = draw_edges(current_edges, e_xys)

        # exit check
        non_visited_inds = np.where(track_edges == 0)[0]
        if non_visited_inds.shape[0] > 0:

            # pick one edge randomly
            k = np.random.choice(non_visited_inds, 1, replace=False)
            ...
            # append next state
            new_track_edges =  np.array(track_edges)
            new_track_edges[k] = 1.0
            to_add = (np.array(current_edges), new_track_edges)
            states.append(to_add)

            if prob > 0.2:
                to_add = (new_state, new_track_edges)                
                states.append(to_add)
        ...
        # draw final state
            ...
            # im_curr = torch.tensor(np.concatenate([im_comb.transpose(2, 0, 1)/255.0, im_s[np.newaxis, :, :]])).float().cuda()
            ...
            #if (tuple(current_edges) not in keep_track):
            #if best_state is None:
                #best_state = im_s
                best_edges = current_edges
            # else:
            #     im_sx = np.concatenate([im_comb.transpose(2, 0, 1)/255.0, best_state[np.newaxis, :, :], im_s[np.newaxis, :, :]])
            #     with torch.no_grad():
            #         _input = torch.from_numpy(im_sx).float().cuda().unsqueeze(0)
            #         probs = _steps(_input)
            #         prob = probs[0][1]
            ...
            #         best_edges = current_edges
                ...
                draw_relations(corners_det/2.0, edges_det/2.0, best_edges, corner_edge, _id)
                dwg.save() 
                count+=1
