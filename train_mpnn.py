...
from model.modules import findLoopsModule, findBestMultiLoopEdge
import copy
            ...
            for pred_index, result in enumerate(results):
                ...
                if visualize and (pred_index == len(results) - 1):
                    ...
                    if index_offset == 0 and options.building_id != '':
                        #loop_edge_masks = debug_info[0]
                        loop_edge_masks = torch.cat([debug_info[0], debug_info[1].float()], dim=0)
                        print((torch.cat([torch.stack([torch.arange(len(edge_pred)).float().cuda(), edge_pred * 100, edge_gt * 100], dim=-1), edge_corner.float()], dim=-1)).round().int())
                        ...
                        for mask_index, edge_mask in enumerate(loop_edge_masks[order].detach().cpu().numpy()):
                            if mask_index == len(debug_info[0]):
                                print('multi loop')
                                pass
                            print(mask_index, edge_mask.nonzero(), (edge_mask * (edge_pred.detach().cpu().numpy() - 0.5)).sum())
                    ...
                    pass
                continue
            images, _ = building.visualize(mode='', edge_state=edge_gt.detach().cpu().numpy() > 0.5, color=[0, 0, 255], debug=True)
            row_images.append(images[0])            
            if sample_index % 500 < 16:
