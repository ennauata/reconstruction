...
def main(options):
    ...
    for num_edges in all_num_edges:
        ...
        if options.task == 'visualize':
            testOneEpoch(options, model, validator, dset_val, visualize=True)
            exit(1)
            pass
...
def testOneEpoch(options, model, validator, dataset, visualize=False):
    #model.eval()
    ...
    for sample_index, sample in enumerate(data_iterator):
        ...
        connection_confidence = validator.validate(dataset.buildings[building_index])
        connections = torch.cat([connections, connection_confidence.unsqueeze(-1)], dim=-1)
        
        corner_pred, connection_pred = model(corners, connections, edge_index, edge_attr)

        corner_loss = F.binary_cross_entropy(corner_pred, corner_gt)
        connection_loss = F.binary_cross_entropy(connection_pred, connection_gt)
        losses = [corner_loss, connection_loss]
